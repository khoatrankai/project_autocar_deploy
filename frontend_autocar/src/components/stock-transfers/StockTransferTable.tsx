...
// --- CẤU HÌNH CỘT ---
const COLUMN_CONFIG = [
  { key: "code", label: "Mã chuyển hàng", default: true },
  { key: "transfer_date", label: "Ngày chuyển", default: true },
  { key: "received_date", label: "Ngày nhận", default: true },
  { key: "from_warehouse", label: "Từ chi nhánh", default: true },
  { key: "to_warehouse", label: "Tới chi nhánh", default: true },
  { key: "total_value", label: "Giá trị chuyển", default: true },
  { key: "status", label: "Trạng thái", default: true },
  { key: "actions", label: "", default: true },
];

export default function StockTransferTable() {
  const { transfers, total, filters, setFilters, fetchTransfers, isLoading } =
  ...
  const [expandedRowId, setExpandedRowId] = useState<string | null>(null); // ID dòng đang mở
  ...
  useEffect(() => {
    setSelectedIds([]);
    setExpandedRowId(null); // Reset expand khi data thay đổi
  }, [transfers]);
  ...
  const handleSelectAll = () => {
    if (
      transfers.length > 0 &&
      transfers.every((t: any) => selectedIds.includes(t.id))
    ) {
      setSelectedIds(
        selectedIds.filter((id) => !transfers.find((t: any) => t.id === id)),
      );
    } else {
      const newIds = transfers.map((t: any) => t.id);
  ...
  const handlePageChange = (newPage: number) => {
    ...
    setExpandedRowId(null); // Đóng detail khi chuyển trang
  };
      ...
      <div className="flex-1 flex flex-col h-full bg-[#f0f2f5] overflow-hidden min-h-[500px]">
        ...
        <div className="p-4 flex justify-between items-center bg-white shadow-sm z-10 border-b border-gray-200">
          ...
          <div className="flex gap-2 items-center">
            ...
            >
              <Plus size={16} /> <span>Chuyển hàng</span>
            </button>
            <button
              onClick={handleImportClick}
              className="flex items-center gap-2 px-3 py-2 bg-white text-gray-700 border rounded hover:bg-gray-50 text-sm"
            >
              <Upload size={16} />
            ...
            <div className="relative" ref={selectorRef}>
              ...
              {showColumnSelector && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded shadow-lg border z-50 p-2">
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="text-xs text-gray-500 uppercase bg-[#f9fafb] border-b border-gray-200 sticky top-0 z-10">
                  <tr>
                    ...
                    {visibleColumns.from_warehouse && (
                      <th className="p-4 font-semibold text-gray-600">
                        Từ chi nhánh
                      </th>
                    )}
                    {visibleColumns.to_warehouse && (
                      <th className="p-4 font-semibold text-gray-600">
                        Tới chi nhánh
                      </th>
                    )}
                    {visibleColumns.total_value && (
                        ...
                        <React.Fragment key={item.id}>
                          ...
                          {expandedRowId === item.id && (
                            <tr>
                              ...
                              >
                                <StockTransferDetailPanel
                                  transferId={item.id}
                                  onClose={() => setExpandedRowId(null)}
                                  onOpenModal={() => setDetailId(item.id)}
                                />
                              </td>
                            </tr>
                          )}
