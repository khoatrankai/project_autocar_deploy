...
// Các cột khớp với Backend (COLUMN_DEFINITIONS trong ImportsService)
const EXPORT_OPTIONS = [
  { id: "code", label: "Mã phiếu" },
  { id: "import_date", label: "Ngày nhập" },
  { id: "supplier_code", label: "Mã NCC" },
  { id: "supplier_name", label: "Tên Nhà cung cấp" },
  { id: "warehouse", label: "Kho nhập" },
  { id: "staff", label: "Người nhập" },
  { id: "total_amount", label: "Tổng tiền" },
  { id: "discount", label: "Giảm giá" },
  { id: "final_amount", label: "Cần trả NCC" },
  { id: "paid_amount", label: "Đã thanh toán" },
  { id: "debt", label: "Còn nợ" },
  { id: "status", label: "Trạng thái" },
  { id: "note", label: "Ghi chú" },
];

export default function ExportPurchaseOrderModal({ isOpen, onClose }: Props) {
  ...
  const handleExport = async () => {
    ...
    setIsLoading(true);
    try {
      const blob = await purchaseOrderService.exportOrders(selectedCols);
      ...
      link.setAttribute(
        "download",
        `phieu_nhap_${new Date().toISOString().slice(0, 10)}.xlsx`,
      );
