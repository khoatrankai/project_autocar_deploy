...
import { usePurchaseOrderStore } from "../../../store/usePurchaseOrderStore"; // Lấy danh sách NCC/Staff
import { useProductStore } from "../../../store/useProductStore"; // Lấy danh sách Kho
...
interface ReturnItem {
  ...
  name: string;
  quantity: number;
  refund_price: number; // Giá trả lại (thường là giá vốn hoặc giá nhập)
  unit: string;
  image_url?: string;
}
...
export default function CreateReturnModal({
  ...
  // Dùng chung store để lấy danh sách nhân viên/NCC
  const {
    filterOptions: purchaseOptions,
    fetchFilterOptions: fetchPurchaseOptions,
  } = usePurchaseOrderStore();
  // Dùng store product để lấy danh sách kho
  const {
    filterOptions: productOptions,
    fetchFilterOptions: fetchProductOptions,
  } = useProductStore();
  ...
  const [formData, setFormData] = useState({
    partner_id: "", // NCC nhận hàng trả
    warehouse_id: "", // Kho xuất trả
    staff_id: "",
    code: "",
    status: "draft",
    discount: 0, // Giảm giá (nếu có, vd: phí phạt trả hàng)
    paid_amount: 0, // NCC đã trả tiền lại
    note: "",
  });
  ...
  useEffect(() => {
    if (isOpen) {
      fetchPurchaseOptions(); // Lấy NCC, Staff
      fetchProductOptions(); // Lấy Warehouse
      setItems([]);
      setFormData({
        partner_id: "",
        warehouse_id: "",
  ...
  const handleAddItem = (product: any) => {
    ...
    const newItem: ReturnItem = {
      ...
      name: product.name,
      quantity: 1,
      // Mặc định lấy giá vốn hoặc giá nhập cuối làm giá trả lại
      refund_price: Number(product.cost_price) || 0,
    ...
    reader.onload = async () => {
      try {
        ...
        worksheet.eachRow((row, rowNumber) => {
          ...
          // Cấu trúc cột tương tự: 2=SKU, 5=SL, 6=Giá hoàn trả
              ...
              .then((res) => {
                ...
                if (product) {
                  newItems.push({
                    ...
                    quantity: quantity,
                    refund_price:
                      price > 0 ? price : Number(product.cost_price),
                    unit: product.unit || "Cái",
                    image_url: product.image_url,
                  });
                  skuSet.add(sku);
  ...
  const handleDownloadTemplate = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Mau_Tra_Hang");
    worksheet.columns = [
      ...
      { header: "Giá hoàn trả", key: "price", width: 15 }, // Đổi label
      { header: "Thành tiền", key: "total", width: 15 },
    ];
    // ... (Styling logic giữ nguyên)
    ...
    saveAs(blob, "Mau_Tra_Hang_Nhap.xlsx");
  };
  ...
  // NCC cần trả = Tổng tiền hàng - Phí (discount)
  const mustPay = Math.max(0, totalRefund - formData.discount);
  ...
  const handleSubmit = async (status: "draft" | "completed") => {
    ...
    try {
      const payload = {
        ...
        // order_id: null, // Trả hàng nhập không cần order_id (theo logic đã bàn)
        note: formData.note,
        status: status,
        // Các trường bổ sung nếu backend hỗ trợ lưu warehouse/staff vào bảng returns hoặc orders ảo
        ...
        // Tiền nong (Backend cần xử lý lưu transaction nếu completed)
      ...
      toast.success(
        status === "draft" ? "Đã lưu phiếu tạm" : "Đã trả hàng thành công!",
      );
      ...
      <div className="w-full h-full bg-white flex flex-col overflow-hidden">
        ...
        <div className="h-14 flex-none flex justify-between items-center px-4 border-b border-gray-200 bg-white z-20 relative">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-bold text-gray-800">Trả hàng nhập</h2>
          </div>
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px]">
            <div className="relative group">
              ...
              <input
                autoFocus
                type="text"
                placeholder="Tìm hàng hóa cần trả (F3)"
        ...
        <div className="flex-1 flex overflow-hidden bg-gray-100">
          ...
          <div className="flex-1 flex flex-col m-3 bg-white rounded shadow-sm overflow-hidden relative">
            <div className="bg-gray-100 border-b border-gray-200 text-gray-600 font-semibold text-sm grid grid-cols-12 gap-2 px-4 py-3 select-none">
              ...
              <div className="col-span-2 text-right">Giá trả lại</div>
            ...
            <div className="p-5 space-y-5 overflow-y-auto flex-1 scrollbar-thin">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <input
                    className="w-40 text-right border border-gray-200 rounded px-2 py-1 outline-none bg-gray-50 text-gray-500"
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  >
                    ...
                    {productOptions?.locations?.map((wh) => (
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  >
                    ...
                    {purchaseOptions?.staffs?.map((staff) => (
              ...
              <div className="space-y-3 pt-4 border-t border-gray-100">
                ...
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-600">Phí/Giảm trừ</span>
                  <input
                    type="number"
                    className="w-32 text-right border-b border-gray-200 outline-none bg-transparent py-1"
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex items-center w-32 border-b border-blue-500">
                    <input
                      type="number"
                      className="w-full text-right outline-none bg-transparent py-1 font-medium text-gray-800"
