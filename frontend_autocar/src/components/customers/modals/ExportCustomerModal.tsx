...
import { customerService } from "../../../services/customerService"; // Import service customer
...
// Các cột cho phép lựa chọn khi xuất file Khách hàng
const EXPORT_OPTIONS = [
  { id: "code", label: "Mã Khách hàng" },
  { id: "name", label: "Tên khách hàng" },
  { id: "phone", label: "Điện thoại" },
  { id: "email", label: "Email" },
  { id: "group_name", label: "Nhóm khách hàng" },
  { id: "address", label: "Địa chỉ" },
  { id: "current_debt", label: "Nợ hiện tại" },
  { id: "total_revenue", label: "Tổng bán" },
  // { id: "net_revenue", label: "Tổng bán trừ trả hàng" }, // Nếu backend hỗ trợ tính toán cột này
  { id: "status", label: "Trạng thái" },
  { id: "notes", label: "Ghi chú" },
  { id: "created_at", label: "Ngày tạo" },
];

export default function ExportCustomerModal({ isOpen, onClose }: Props) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `customers_export_${new Date().toISOString().slice(0, 10)}.xlsx`,
      );
